...
Public Class frmDespesas
    ...
    Private Sub formata_grid_itens()
        ...
        grdItens.DataSource = Lancamento.ListarDespesasData(dtIni.Value.ToShortDateString(), dtFim.Value.ToShortDateString(), conf.Filial)

        'Verifica se há registro, se houver habila os botões de Editar e Excluir
        If Lancamento.ListarDespesasData(dtIni.Value.ToShortDateString(), dtFim.Value.ToShortDateString(), conf.Filial).Rows.Count > 0 Then
            txtTotal.Text = String.Format("{0:##0.00}", Lancamento.TotalDespesasData(dtIni.Value.ToShortDateString(), dtFim.Value.ToShortDateString(), conf.Filial))
    ...
    Private Sub combos()
        ...
        "like '3%' or classificacao like '2%' or classificacao like '4%') " &
    ...
    Private Sub btnNovo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNovo.Click
        If caixa.convertedata(caixa.retornaDataCaixaAberto(conf.Filial)) = caixa.convertedata(caixa.retornaDataHoraServidor) Then
            ...
            habilitaControle()
        Else
            MessageBox.Show("Não é possível lançar despesas com caixa anterior aberto.", "INFORMAÇÃO", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    End Sub
        ...
        If caixa.retornaDataCaixaExistente(CDate(caixa.convertedata(dtLancamento.Value))) = True Then
            If caixa.retornaDataCaixaAberto(CDate(caixa.convertedata(dtLancamento.Value))) = False Then
                ...
                Else
                    ...
                    Lancamento.Acrescimo = 0.0
                    Lancamento.Juros = 0.0
                    Lancamento.Desconto = 0.0
                    Lancamento.Taxa = 0.0
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimir.Click
        If Lancamento.ListarDespesasData(dtIni.Value, dtFim.Value, conf.Filial).Rows.Count = 0 Then
            MessageBox.Show("Não há despesa a ser exibida.", "INFORMAÇÃO", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If

        Dim f As New frmRpt
        Dim r As New rptDespesas
        r.DataSource = Lancamento.ListarDespesasData(dtIni.Value, dtFim.Value, conf.Filial)
        r.diai = dtIni.Value
        r.diaf = dtFim.Value
        f.Relat(r)
        f.ShowDialog(Me)
    ...
    Private Sub grdItens_Click(sender As Object, e As EventArgs) Handles grdItens.Click
        Lancamento.RetornaRegistro(grdItens.CurrentRow.Cells(0).Value, "S")
    ...
    Private Sub dtFim_Enter(sender As System.Object, e As System.EventArgs) Handles dtFim.Enter
        If Lancamento.ListarDespesasData(dtIni.Value.ToShortDateString(), dtFim.Value.ToShortDateString(), conf.Filial).Rows.Count = 0 Then
            btnNovo.Focus()
        End If
    End Sub
